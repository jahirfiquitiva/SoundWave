...
function login() {
    ...
    if (name.length > 0 && pass.length > 0) {
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                if (xhr.responseText.length > 0) {
                    ...
                    if (jsonContent.name !== undefined) {
                        M.toast({
                                    html: "Bienvenido " + jsonContent.name,
                                    displayLength: 2000
                                });
                        ...
                        M.toast({html: "Error!<br>" + jsonContent.error, displayLength: 2000});
                    }
                }
            }
        };
        ...
        M.toast({html: "Los campos estan vacios.", displayLength: 2000});
    }
}
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                if (xhr.responseText.length > 0) {
                    ...
                    if (json.code === 4) {
                        M.toast({html: "Usuario añadido con exito!", displayLength: 2000});
                        ...
                        M.toast({html: "Error!<br>" + json.error, displayLength: 2000});
                    }
                }
            }
        };
...
function logout() {
    M.toast({html: "Adios " + document.getElementById("user-name").innerHTML, displayLength: 2000});
    document.getElementById("username").value = "";
    document.getElementById("password").value = "";
